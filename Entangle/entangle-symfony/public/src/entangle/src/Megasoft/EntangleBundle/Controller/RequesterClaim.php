...
class Claim extends Controller {
    ...
    /**
     * This function gets the emails of both the requester and the tangle owner from 
    ...
    public function getMails($request, $requestId) {
        ...
        $requesterRequest = $requestRepo->findOneBy(array('id' => $requestId));

        if ($requesterRequest == null) {
            return new Response('Bad Request', 400);
        }
        $tangleId = $requesterRequest->getTangleId();
        ...
        $requesterMail = $userRepo->findOneBy(array('userId' => $userId))->getEmail();
        ...
        $response->setJsonContent
                (array('X-TANGLEOWNER-MAIL' => $tangleOwnerMail, 'X-CLAIMER-MAIL' => $requesterMail));
